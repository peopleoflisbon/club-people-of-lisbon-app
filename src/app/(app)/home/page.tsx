...
export default async function HomePage() {
  ...
  const [
    ...
    (supabase as any).from('member_events').select('id, name, event_date, event_time, event_end_date, event_end_time, location, submitted_by, image_url').gte('event_date', new Date().toISOString().split('T')[0]).order('event_date', { ascending: true }).limit(3),
