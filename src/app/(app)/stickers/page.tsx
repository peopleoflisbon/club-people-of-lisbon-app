...
export default function StickersPage() {
  ...
  const [enlarged, setEnlarged] = useState<AlbumSticker | null>(null);
    ...
    <>
      <ScrollPage>
        <div style={{ maxWidth: 480, margin: '0 auto', fontFamily: FF }}>
          ...
          {loading ? (
            <div style={{ padding: '48px 0', textAlign: 'center', color: MUTED, fontSize: 13 }}>Loading your collection…</div>
          ) : data && (
...
function Section({ title, stickers, onEnlarge }: { title: string; stickers: AlbumSticker[]; onEnlarge: (s: AlbumSticker) => void }) {
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      <div style={{ background: '#EDE7DC', borderRadius: 2, height: 3, marginBottom: 12, overflow: 'hidden' }}>
        <div style={{
          background: RED, height: '100%', borderRadius: 2,
