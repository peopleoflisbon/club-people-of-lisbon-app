...
export async function GET() {
  ...
  const applyOverride = (type: string, source_id: string, name: string, subtitle: string, description: string = '') => {
    const o = overrideMap.get(`${type}:${source_id}`);
    return {
      custom_name: o?.custom_name ?? null,
      custom_subtitle: o?.custom_subtitle ?? null,
      custom_description: o?.custom_description ?? null,
      ...
      display_description: (o?.custom_description !== undefined && o?.custom_description !== null) ? o.custom_description : description,
    };
  };
  ...
  const customStickers = (custom || []).map((c: any, i: number) => ({
    ...
    ...applyOverride(c.type, c.id, c.name, c.subtitle || '', c.description || ''),
  }));
...
export async function PATCH(req: Request) {
  const supabase = createRouteHandlerClient({ cookies });
  if (!await verifyAdmin(supabase)) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  const { sticker_type, source_id, custom_name, custom_subtitle, custom_description } = await req.json();
  ...
  const { error } = await db.from('sticker_overrides').upsert({
    sticker_type, source_id, custom_name, custom_subtitle, custom_description,
