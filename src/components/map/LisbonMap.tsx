...
import type { MapPin, Category } from '@/types';
...
function getMapsUrl(address: string): string {
  return `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address + ', Lisbon, Portugal')}`;
}
...
export default function LisbonMap({ pins, isMapUser = false, categories = [] }: Props) {
  ...
  // First-time overlay for map_users
  useEffect(() => {
    if (!isMapUser) return;
  ...
  // Filtered pins — by active category IDs
  ...
  // Add markers when map ready or filtered pins change
  useEffect(() => {
  ...
  // Get category names for selected pin
  const selectedPinCategories = selectedPin
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
      ...
      {/* ── Top-left: Exit / Explore ── */}
      <div className="absolute left-4 z-10" style={{ top: safeTop }}>
        <button
          onClick={() => isMapUser ? setShowExitPrompt(true) : undefined}
          style={{
            display: 'flex', alignItems: 'center', gap: 8,
            padding: '10px 16px', borderRadius: 12,
            background: 'rgba(250,248,244,0.93)', backdropFilter: 'blur(12px)',
            boxShadow: '0 4px 20px rgba(0,0,0,0.1)',
            border: 'none', cursor: isMapUser ? 'pointer' : 'default',
          }}>
          {isMapUser && (
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#2F6DA5" strokeWidth="2.5">
              <path d="M19 12H5M12 5l-7 7 7 7"/>
            </svg>
          )}
          <div>
            <p style={{ fontSize: 12, fontWeight: 700, color: '#2F6DA5', margin: 0 }}>
              {isMapUser ? 'Exit map' : 'Explore Lisbon'}
            </p>
            {!isMapUser && (
              <p style={{ fontSize: 10, color: '#A89A8C', margin: 0 }}>Through the people we've filmed</p>
            )}
          </div>
        </button>
      </div>

      {/* ── Top-right: Join the Club + filter + stories ── */}
        ...
        {isMapUser && (
          <a href="https://www.peopleoflisbon.com" target="_blank" rel="noopener noreferrer"
            style={{
              ...
              padding: '10px 18px', background: '#2F6DA5', color: 'white',
              borderRadius: 999, fontSize: 13, fontWeight: 700,
              textDecoration: 'none', boxShadow: '0 2px 12px rgba(47,109,165,0.45)',
            }}>
            Join the Club ↗
          </a>
        )}
          ...
          {categories.length > 0 && (
            <button onClick={() => setShowFilters(true)}
              style={{
                ...
                background: hasFilters ? '#2F6DA5' : 'rgba(250,248,244,0.93)',
                backdropFilter: 'blur(12px)',
                boxShadow: '0 2px 12px rgba(0,0,0,0.08)',
                border: 'none', cursor: 'pointer',
              }}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none"
                stroke={hasFilters ? 'white' : '#2F6DA5'} strokeWidth="2.5">
          ...
          {pins.length > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 12px', background: 'rgba(250,248,244,0.93)', backdropFilter: 'blur(12px)', borderRadius: 999, boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 600, color: '#1C1C1C' }}>
                {filteredPins.length}{hasFilters ? `/${pins.length}` : ''} {pins.length === 1 ? 'story' : 'stories'}
              </span>
            </div>
          )}
        </div>
            ...
            <div style={{ background: 'rgba(250,248,244,0.98)', backdropFilter: 'blur(20px)', borderRadius: '20px 20px 0 0', boxShadow: '0 -8px 40px rgba(0,0,0,0.15)', overflow: 'hidden' }}
              ...
              <div style={{ padding: '14px 18px 18px' }}>
                ...
                {selectedPin.filmed_address && (
                  <a href={getMapsUrl(selectedPin.filmed_address)} target="_blank" rel="noopener noreferrer"
                    style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: '#2F6DA5', textDecoration: 'none', marginBottom: 12 }}>
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#2F6DA5" strokeWidth="2">
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  ...
                  {selectedPin.filmed_address && (
                    <a href={getMapsUrl(selectedPin.filmed_address)} target="_blank" rel="noopener noreferrer"
                      ...
                      style={{ background: '#EEF4FA', color: '#2F6DA5', textDecoration: 'none' }}>
          ...
          <div style={{ background: '#FAFAF8', borderRadius: '20px 20px 0 0', padding: '20px 20px 32px', width: '100%', maxWidth: 440, boxShadow: '0 -8px 40px rgba(0,0,0,0.12)' }}
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowFilters(false)}
                style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4, color: '#A89A8C', fontSize: 20, lineHeight: 1 }}>✕</button>
            </div>
      ...
      {/* ── Exit map prompt ── */}
      {showExitPrompt && isMapUser && (
        <div className="fixed inset-0 z-50 flex items-end justify-center lg:items-center"
          style={{ background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }}>
          <div style={{ background: 'rgba(10,10,15,0.92)', backdropFilter: 'blur(24px)', borderRadius: '20px 20px 0 0', padding: '28px 24px 32px', width: '100%', maxWidth: 420, border: '1px solid rgba(255,255,255,0.1)' }}
            className="lg:rounded-2xl lg:mx-4">
            <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', marginBottom: 10 }}>Before you go…</p>
            <h2 style={{ fontSize: 22, fontWeight: 700, color: '#fff', margin: '0 0 10px', lineHeight: 1.2 }}>Join Club People Of Lisbon</h2>
            <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.55)', lineHeight: 1.65, margin: '0 0 22px' }}>
              Unlock events, members, and the full Lisbon experience.
            </p>
            <a href="https://www.peopleoflisbon.com" target="_blank" rel="noopener noreferrer"
              style={{ display: 'block', width: '100%', padding: '13px', background: '#2F6DA5', color: 'white', fontSize: 15, fontWeight: 700, borderRadius: 10, textAlign: 'center', textDecoration: 'none', marginBottom: 10 }}>
              Join the Club ↗
            ...
            <button onClick={handleSignOut}
              style={{ display: 'block', width: '100%', padding: '12px', background: 'rgba(255,255,255,0.07)', color: 'rgba(255,255,255,0.55)', fontSize: 14, fontWeight: 600, borderRadius: 10, border: '1px solid rgba(255,255,255,0.1)', cursor: 'pointer', marginBottom: 8 }}>
              Continue leaving
            </button>
            <button onClick={() => setShowExitPrompt(false)}
              style={{ display: 'block', width: '100%', padding: '8px', background: 'none', color: 'rgba(255,255,255,0.3)', fontSize: 13, border: 'none', cursor: 'pointer' }}>
              Stay and explore
            </button>
          </div>
        </div>
